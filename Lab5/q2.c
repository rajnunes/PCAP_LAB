...
// Create the two input vectors
	...
	scanf("%d",&LIST_SIZE);
	unsigned int *A = (int*)malloc(sizeof(int)*LIST_SIZE);
	...
	cl_mem memA = clCreateBuffer(context,
		CL_MEM_READ_ONLY,LIST_SIZE * sizeof(unsigned int), NULL, &ret);
	cl_mem memB = clCreateBuffer(context,
		CL_MEM_READ_ONLY,LIST_SIZE * sizeof(unsigned int), NULL, &ret);
	// Copy the lists A and B to their respective memory buffers
	ret = clEnqueueWriteBuffer(command_queue, memA, CL_TRUE, 0,LIST_SIZE
		* sizeof(unsigned  int), A, 0, NULL, NULL);
	...
	cl_kernel kernel = clCreateKernel(program, "oneCompliment", &ret);
	...
	unsigned int *B = (int*)malloc(sizeof(int)*LIST_SIZE);
	/*for (int i = 0; i < LIST_SIZE; ++i)
	{
		printf("%d\n",B[i] );
	}
	printf("\n");*/
	ret = clEnqueueReadBuffer(command_queue, memB, CL_TRUE, 0,LIST_SIZE
